	...
	return (
		...
		>
			<Card className="overflow-hidden h-full flex flex-col">
				<div className="aspect-video relative overflow-hidden" style={{ perspective: '1000px' }}>
					<motion.img
						src={image}
						alt={title}
					...
					{category === 'Upcoming' && (
						<div className="space-y-4">
							...
							{waitlistCount !== undefined && (
								<p className="text-sm text-muted-foreground">{waitlistCount} users waitlisted</p>
							)}
							{progress !== undefined && (
							...
							{onNotify && (
								<Button onClick={onNotify} className="w-full" variant="outline">
									Notify Me
								</Button>
							)}
						</div>
